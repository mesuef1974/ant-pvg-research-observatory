"""تحليل مستودع الموسوعة: LaTeX وسجلات النتائج والببليوغرافيا والمعرفة المعيارية.

وحدة مجال نقية: لا تعرف قاعدة بيانات ولا ORM ولا HTTP. تُرجع بنى بايثون
عادية، فتبقى قابلة للاختبار وحدها ولإعادة الاستعمال خارج الخادم.

المصدر المعتمد هو مصدر LaTeX لا ملف PDF المُصدَّر: استخراج العربية من PDF
يعطي نصًا معكوسًا بلا فواصل كلمات فيتعذر البحث، بينما المصدر UTF-8 نظيف
ويحمل معه البنية والمعرفات الثابتة ANT-* وحالاتها.
"""
import os
import re
import unicodedata
...
_MATH_ENVS = ('align\\*?|equation\\*?|gather\\*?|multline\\*?|aligned|cases'
              '|pmatrix|bmatrix|tabular|array')
...
def parse_chapter(path, root):
    ...
    if not marks:
        ...
        for i, (_start, en) in enumerate(marks):
...
MODEL_DIR = REPO_ROOT/'knowledge'/'model_synthesis'
